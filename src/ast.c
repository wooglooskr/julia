...
static void jl_init_ast_ctx(jl_ast_context_t *ast_ctx)
{
    ...
    jl_ast_ctx(fl_ctx)->ssaval_sym = symbol(fl_ctx, "ssaval");
...
static jl_value_t *scm_to_julia_(fl_context_t *fl_ctx, value_t e, int eo)
{
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (e == fl_ctx->NIL) {
            ...
            hd = car_(e);
            if (hd == jl_ast_ctx(fl_ctx)->ssaval_sym)
                return jl_box_ssaval(numval(car_(cdr_(e))));
...
static value_t julia_to_scm_(fl_context_t *fl_ctx, jl_value_t *v)
{
    ...
    if (jl_is_ssaval(v))
        return fl_list2(fl_ctx, jl_ast_ctx(fl_ctx)->ssaval_sym, fixnum((size_t)((jl_ssaval_t*)v)->id));
    if (v == jl_true)
        return fl_ctx->T;
    if (v == jl_false)
        return fl_ctx->F;
...
jl_lambda_info_t *jl_wrap_expr(jl_value_t *expr)
{
    ...
    jl_cellset(vi, 0, mt);
    jl_cellset(vi, 1, mt);
    // front end always wraps toplevel exprs with ssavals in (thunk (lambda () ...))
...
// get array of types for SSAVals, or its length (if not type-inferred)
jl_value_t *jl_lam_ssavals(jl_expr_t *l)
{
    assert(jl_is_expr(l));
    jl_value_t *le = jl_exprarg(l, 1);
    assert(jl_is_array(le));
