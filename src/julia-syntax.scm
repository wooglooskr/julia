...
(define (effect-free? e)
  (or (not (pair? e)) (ssaval? e) (sym-dot? e) (quoted? e) (equal? e '(null))))
   ...
   (receive
    ...
    (begin
      ...
      (let* ((iscall (is-call-name? name))
             ...
             ;; HACK: the typevars need to be bound to ssavals, since this code
             ;; might be moved to a different scope by closure-convert.
             (temps (map (lambda (x) (make-ssaval)) names))
      ...
      (let (;; expand inner try blocks first, so their return statements
        ...
        (let ((hasret (or (contains return? tryb)
          ...
          (let ((err (gensy))
                ...
                (val (gensy))) ;; this is an ssaval, but llvm has trouble determining that it dominates all uses
...
(define (expand-forms e)
  (if (or (atom? e) (memq (car e) '(quote inert top line module toplevel ssaval null meta)))
      e
      ...
      ((assignment? (caddr e))
       ...
       (let loop ((lhss (list lhs))
         ...
         (if (assignment? rhs)
             ...
             (let ((rr (if (symbol-like? rhs) rhs (make-ssaval))))
               (expand-forms
       ...
       (case (car lhs)
         ((|.|)
          ...
          (let ((a   (cadr (cadr e)))
                (b   (caddr (cadr e)))
                (rhs (caddr e)))
            (let ((aa (if (symbol-like? a) a (make-ssaval)))
                  (bb (if (or (atom? b) (symbol-like? b) (and (pair? b) (quoted? b)))
                          b (make-ssaval)))
                  (rr (if (or (symbol-like? rhs) (atom? rhs)) rhs (make-ssaval))))
              `(block
         ...
         ((ref)
          ...
          (let ((a    (cadr lhs))
            ...
            (let* ((reuse (and (pair? a)
                   ...
                   (rrhs (and (pair? rhs) (not (ssaval? rhs)) (not (quoted? rhs))))
                   (r    (if rrhs (make-ssaval) rhs))
...
(define (lower-comprehension atype expr ranges)
  ...
  (let ((result    (make-ssaval))
        (ri        (gensy))
        (initlabl  (if atype #f (make-ssaval)))
        (oneresult (make-ssaval))
        (lengths   (map (lambda (x) (make-ssaval)) ranges))
        (states    (map (lambda (x) (gensy)) ranges))
        (is        (map (lambda (x) (gensy)) ranges))
        (rv        (map (lambda (x) (make-ssaval)) ranges)))
...
(define (lower-typed-dict-comprehension atypes expr ranges)
  (if (not (and (length= atypes 3)
      ...
      (let ( (result (make-ssaval))
             (rs (map (lambda (x) (make-ssaval)) ranges)) )
...
;; in-place to
;;   (var-info-lst captured-var-infos ssavals static_params)
...
(define (convert-assignment var rhs0 fname lam interp)
  (let* ((vi (assq var (car  (lam:vinfo lam))))
    ...
    (if (and (not closed) (not capt) (eq? vt 'Any))
        `(= ,var ,rhs0)
        (let* ((rhs1 (if (or (ssaval? rhs0) (simple-atom? rhs0)
           ...
           (let* ((name  (method-expr-name e))
             ...
             (if (not local?) ;; not a local function; will not be closure converted to a new type
                 ...
                 (let* ((exists (get namemap name #f))
                        ...
                        (method-sp (map (lambda (s) (make-ssaval)) capt-sp))
...
(define (compile-body e vi lam)
  (let ((code '())
    ...
    (define (emit-return x)
      (if (> handler-level 0)
          (let ((tmp (if (or (simple-atom? x) (ssaval? x) (equal? x '(null)))
                         #f (make-ssaval))))
            (if tmp (emit `(= ,tmp ,x)))
      ...
      (if (null? lst) '()
          (let ((temps? (or *very-linear-mode*
                            (expr-contains-p (lambda (x) (and (assignment? x)
                                                              (symbol? (cadr x))))
                                             (cons 'block (cdr lst)))))
                (simple? (every (lambda (x) (or (simple-atom? x) (symbol? x) (ssaval? x)
                                                (and (pair? x)
                  ...
                  (let* ((arg (car lst))
                    ...
                    (loop (cdr lst)
                          (cons (if (and temps? (not simple?)
                                         (not (simple-atom? arg))  (not (ssaval? arg))
                                         (not (simple-atom? aval)) (not (ssaval? aval))
                                         (not (and (pair? arg)
    ...
    (define (compile-cond ex break-labels)
      (let ((cnd (compile ex break-labels #t #f)))
        (if (and *very-linear-mode*
                 (not (or (simple-atom? cnd) (ssaval? cnd) (symbol? cnd))))
            (let ((tmp (make-ssaval)))
              (emit `(= ,tmp ,cnd))
              tmp)
            cnd)))
    ...
    (define (compile e break-labels value tail)
      (if (or (not (pair? e)) (memq (car e) '(null ssaval quote inert top copyast the_exception $
          ...
          (case (car e)
            ...
            ((=)
             (let ((rhs (compile (caddr e) break-labels #t #f)))
               (if value
                   (let ((rr (if (or (atom? rhs) (ssaval? rhs) (eq? (car rhs) 'null))
                                 rhs (make-ssaval))))
                     (if (not (eq? rr rhs))
                         (emit `(= ,rr ,rhs)))
                     (emit `(= ,(cadr e) ,rr))
...
(define (renumber-lambda lam)
  ...
  (define (renumber-slots e)
    (cond ((symbol? e)
          ...
          ((ssaval? e)
           (let ((idx (or (get ssaval-table (cadr e) #f)
                          (begin0 nssavals
                                  (put! ssaval-table (cadr e) nssavals)
                                  (set! nssavals (+ nssavals 1))))))
             `(ssaval ,idx)))
          ((eq? (car e) 'lambda)
           (renumber-lambda e))
          (else (cons (car e)
  ...
  (let ((body (renumber-slots (lam:body lam)))
        (vi   (lam:vinfo lam)))
    `(lambda ,(cadr lam)
       (,(car vi) ,(cadr vi) ,nssavals ,(last vi))
       ,body)))
