...
static int array_resize_buffer(jl_array_t *a, size_t newlen)
{
...
static inline void jl_array_grow_at_beg(jl_array_t *a, size_t idx, size_t inc)
{
    ...
    if (a->offset >= inc) {
        ...
        if (inc > (a->maxsize - n) / 2 - (a->maxsize - n) / 20) {
            ...
            a->offset = (newlen - newnrows) / 2;
            newdata = (char*)a->data + a->offset * elsz;
        }
        else {
            assert(!a->flags.isshared);
    ...
    a->data = newdata;
    if (a->flags.ptrarray) {
        memset(newdata + nb1, 0, nbinc);
    } else if (elsz == 1) {
        newdata[newnrows] = 0;
    }
}

static inline void jl_array_grow_at_end(jl_array_t *a, size_t idx, size_t inc)
{
...
static inline void jl_array_del_at_beg(jl_array_t *a, size_t idx, size_t dec)
{
    // no error checking
...
static inline void jl_array_del_at_end(jl_array_t *a, size_t idx, size_t dec)
{
    // no error checking
    // assume inbounds, assume unshared
    assert(!a->flags.isshared);
...
JL_DLLEXPORT void jl_array_del_at(jl_array_t *a, ssize_t idx, size_t dec)
{
    ...
    if (__unlikely(a->flags.isshared))
        array_try_unshare(a);
    if (idx < n - last) {
        jl_array_del_at_beg(a, idx, dec);
    }
    else {
        jl_array_del_at_end(a, idx, dec);
    }
}

JL_DLLEXPORT void jl_array_del_beg(jl_array_t *a, size_t dec)
{
    if (__unlikely(dec > jl_array_nrows(a)))
        jl_bounds_error_int((jl_value_t*)a, dec);
    if (__unlikely(a->flags.isshared))
        array_try_unshare(a);
    jl_array_del_at_beg(a, 0, dec);
}
...
JL_DLLEXPORT void jl_array_del_end(jl_array_t *a, size_t dec)
{
    ...
    if (__unlikely(a->flags.isshared))
        array_try_unshare(a);
    jl_array_del_at_end(a, n - dec, dec);
}
