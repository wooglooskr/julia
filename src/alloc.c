...
void jl_lambda_info_set_ast(jl_lambda_info_t *li, jl_value_t *ast)
{
    ...
    jl_value_t *ssaval_types = jl_lam_ssavals((jl_expr_t*)ast);
    assert(jl_is_long(ssaval_types));
    size_t nssaval = jl_unbox_long(ssaval_types);
    ...
    li->ssavaltypes = jl_box_long(nssaval);
    jl_gc_wb(li, li->ssavaltypes);
    int i;
    for(i=0; i < nslots; i++) {
...
JL_DLLEXPORT jl_lambda_info_t *jl_new_lambda_info_uninit(jl_svec_t *sparam_syms)
{
    ...
    li->slottypes = li->ssavaltypes = NULL;
...
static jl_lambda_info_t *jl_clone_thunk(jl_lambda_info_t *linfo, jl_tupletype_t *types)
{
    ...
    new_linfo->ssavaltypes = linfo->ssavaltypes;
...
#ifdef _P64
        boxed_ssaval_cache[i] = jl_box64(jl_ssaval_type, i);
#else
        boxed_ssaval_cache[i] = jl_box32(jl_ssaval_type, i);
#endif
    }
    for(i=0; i < 256; i++) {
...
void jl_mark_box_caches(void)
{
    ...
    for(i=0; i < NBOX_C; i++) {
        ...
        jl_gc_setmark(boxed_ssaval_cache[i]);
    }
}
